    ...
    while (client.connected())
    {
      ...
      { // client data available to read
        ...
        if (c == '\n' && currentLineIsBlank)
        {
          ...
          client.println("background-repeat:repeat-x; background-attachment:fixed;font-family:'Open Sans', sans-serif; background-position:bottom left; background-color: #e6f7ff;}");
          ...
          client.println("body .ondisplay section{ width: 120px; height: 70px; background: #e6f7ff; display: inline-block; position: relative; text-align: center; margin-top: 5px;}");
          client.println("body .ondisplay section:after{content: attr(title); position: absolute; width: 100%; left:0; bottom: 3px; font-size: 12px; font-weight: 400;}");
          client.println("</style>");
...
void ProcessCheckbox(EthernetClient cl)
{
  ...
  cl.println("<label for=\"TEMPERATURE\"></label></div></section></div></div>");
  ...
  if (1) //only if DHT sensor is active
  {
    ...
    else
    {
      ...
      cl.println("<p><div align=\"center\"><input type=\"submit\"class=\"button disabled\" value=\"Submit\" disabled></div></p>");
    }
    cl.println("</div>");
  }
